...
public function addRoleUsr():void
{
	...
	for(var  i:int=0; i<addUserArr.length; i++  )
	{
		if(i == addUserArr.length-1)
		{
			name +=  addUserArr[i].cname ;
			code +=  addUserArr[i].iid ;
		}
		else
		{
			name +=  addUserArr[i].cname + ",";
	...
	EventDispatcherFactory.getEventDispatcher().dispatchEvent(
		new TwitterEvent(TwitterEvent.onTypeViewPersonSelected, object) );
