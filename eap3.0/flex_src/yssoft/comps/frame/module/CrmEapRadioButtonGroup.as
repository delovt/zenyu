...
public class CrmEapRadioButtonGroup extends HBox implements IAllCrmInput {
    ...
    public var radiobutonGroup:RadioButtonGroup = new RadioButtonGroup();
    ...
    public function CrmEapRadioButtonGroup() {
        super();
    }
    ...
    override protected function createChildren():void {
        ...
        if (Boolean(this._singleType.bread)) {
            ...
            //焦点离开事件
            this.radiobutonGroup.addEventListener(Event.CHANGE, onChange);
        }
    ...
    private function consultInitBack(event:ResultEvent):void {
        ...
        if (result.hasOwnProperty("consultList")) {
            ...
            for (var i:int = 0; i < this._consultList.length; i++) {
                var obj:Object = _consultList.getItemAt(i);
                var radio:RadioButton = new RadioButton();
                radio.label = obj[this._singleType.cconsultswfld];
                radio.value = obj[this._singleType.cconsultbkfld];
                radio.group = this.radiobutonGroup;
                if (_singleType.curButtonStatus == "onGiveUp") {
                    radio.enabled = false;
                }
                else {
                    radio.enabled = true;
            ...
            if (CRMtool.isStringNotNull(this._singleType.cnewdefault) && this._singleType.curButtonStatus == "onNew") {
                this.radiobutonGroup.selectedValue = CRMtool.defaultValue(this._singleType.cnewdefault);
            }
            else if (CRMtool.isStringNotNull(this._singleType.ceditdefault) && this._singleType.curButtonStatus == "onEdit") {
                this.radiobutonGroup.selectedValue = CRMtool.defaultValue(this._singleType.ceditdefault);
            }
    ...
    private function onChange(event:Event):void {
		...
		//金税收费单  单击是按钮，将税控收费editable设为true   by ln  2014-03-27
		var crmEap:CrmEapRadianVbox = this.parent.parent.parent.parent.parent as CrmEapRadianVbox;

        if((crmEap.formIfunIid == 390 && name=='ui_tr_charge_brevenue')) {
			if(radiobutonGroup.selectedValue == 1) {
				for each(var textInput:CrmEapTextInput in (crmEap.textInputList)){
					if(textInput.name == 'ui_tr_charge_frevenue'){	
                    ...
                    if (null != _consultList) {
                        ...
                        if (tableList != null) {
                            for (var j:int = 0; j < cfieldRelationshipList.length; j++) {
                                ...
                                if (ctable2.toLocaleUpperCase() == ctable.toLocaleUpperCase()) {
                                    var index:int = 0;
                                    for (var k:int = 0; k < _consultList.length; k++) {
                                        if (_consultList.getItemAt(k)[this._singleType.cconsultswfld + ""] == this.radiobutonGroup.selectedValue) {
                                            index = k;
                                            break;
                                        }
                                    }
                                    tableList.getItemAt(tableList.getItemIndex(dataObj))[String(cfieldRelationship.cfield2)] = _consultList.getItemAt(index)[String(cfieldRelationship.cfield)];
                                    if (int(cfieldRelationship.iconsult) > 0) {
                                        tableList.getItemAt(tableList.getItemIndex(dataObj))[cfieldRelationship.cfield2 + "_Name"] = _consultList.getItemAt(index)[cfieldRelationship.cconsultswfld];
                                    }
                                }
                            }
                            tableList.getItemAt(tableList.getItemIndex(dataObj)).iinvoices = _consultList.getItemAt(index).iid;
    ...
    public function set cvalue(value:String) {
        radiobutonGroup.selectedValue = value;
        _selectedValue = value;
    }

    public function get cvalue():String {
        return radiobutonGroup.selectedValue == null ? _selectedValue : radiobutonGroup.selectedValue + "";
    }
}
}