...
package yssoft.preload
{
	 ...
	 */
	public class CrmDownloadProgressBar extends Sprite implements IPreloaderDisplay
	{
		 ...
		 *  @productversion Flex 3
		 */
		public function CrmDownloadProgressBar() 
		{
			super();
			
			showPercentage = true;
			downloadingLabel="及时、专业、真诚";
			initializingLabel="整合、灵动、高效";
		}
		 ...
		 *  @productversion Flex 3
		 */
		protected var DOWNLOAD_PERCENTAGE:uint = 77;
		...
		{
			return new RoundedRectangle(14, 40, 254, 14);
		}
		...
		{
			return new RoundedRectangle(14, 39, 254, 16, 0);
		}
		...
		{
			return new RoundedRectangle(0, 0, 282, 65, 3);
		}
		...
		protected function set label(value:String):void
		{
			if (!(value is Function)){
				if(value!=downloadingLabel&&value!=initializingLabel)
					_label=downloadingLabel;
				else
					_label=value;
			}
		...
		{
			return new Rectangle(14, 17, 270, 22);
		}
...
/*			g.beginFill(0xCCCCCC, 0.4); 
			...
			_percentObj.x = calcX(_percentRect.x)+125;
			_percentObj.y = calcY(_percentRect.y)+15;
...
/*			var ds:RectangularDropShadow = new RectangularDropShadow();
		...
		private function drawProgressBar(percentage:Number):void
		{
			...
			var colors:Array = [ 0xAAAAAA, 0xAAAAAA ];
			var ratios:Array = [ 0, 0xAA ];
			var matrix:Matrix = new Matrix();
...
import yssoft.preload.CrmDownloadProgressBar;
...
{
	private var downloadProgressBar:CrmDownloadProgressBar;
	 ...
	 */ 
	public function ErrorField(downloadProgressBar:CrmDownloadProgressBar)
	{
		super();
